...
public class SalesforcePage extends BasePage {
	...
	String xpath2 = "//label[contains(text(), '$$program_name$$')]/ancestor::div[@class = 'box-title']";
	...
	{
		clickElement(driver, other_address_box_selection);
	}
	...
	public void click_sendswagbutton() throws Exception
	{
		System.out.println("we are inside click sendswagbutton method");
		clickElement(driver, send_marketing_button);
	}
	...
	public void opencontact(String first_name, String last_name)
	{
		try {
			...
			String full_name_wc = last_name + ", "+ first_name;
			//td//a[contains(.,'"+full_name_wc+"')]
			//driver.findElement(By.xpath("//span[contains(text(), '" + first_name + "')]/ancestor::div[contains(@id, 'FULL_NAME')]/a")).click();
			driver.findElement(By.xpath("//td//a[contains(.,'"+full_name_wc+"')]")).click();
	...
	{   
		...
		System.out.println("clicked on lighting launcher successfully");
	...
	{
		System.out.println("going to select the PFL view");
		clickElement(driver, list_view_drop_down);
		driver.findElement(By.xpath("//option[contains(text(),'"+viewname+"')]")).click();
		System.out.println("selected the view");
		Thread.sleep(5000);
		//WebElement go_b = driver.findElement(go_button);
//		boolean status = driver.findElement(go_button).isDisplayed();
//		System.out.println("go button appears " +status);
//		if(status)
//		{
//			clickElement(driver, go_button);
//			System.out.println("clicked on go button successfully");
//		}
//		else
//		{
//			System.out.println("no need to click on go button");
//		}
	...
	public void verify_program_on_contact_send_marketing_page_new(String program_name, String product_name) throws Exception
	{
		String program_title = program_name + " - " + product_name;
		switchToFrame(driver);
		waittillinvisibility(driver, loading_configuration);
		...
		enterText(driver, set_product_filter_txtbox, program_title);
		System.out.println("entered the text in box successfully");
		WebElement searched_element = driver.findElement(By.xpath(xpath2.replace("$$program_name$$",program_name)));
		System.out.println("element captured succesfully, searched_element");
	...
	public void click_program_on_send_swag_screen(String program_name, String product_name) throws Exception
	{
		...
		driver.findElement(By.xpath("//label[contains(text(), '"+program_name+"')]/ancestor::div[@class = 'box-title']")).click();
	...
	public void switch_back_to_classicmode() throws Exception
	{
		if(check_element_visibility(sflt_logout_button_image))
		{		
			driver.switchTo().defaultContent();
			wait_min();
			clickElement(driver, sflt_logout_button_image);
			clickElement(driver, switch_to_classic);
			System.out.println("switching back to classic mode");
		}
		else 
		{
			System.out.println("already on classic mode");
		}
	}

	public void verify_lightning_page(By locator) throws Exception
	{
		boolean status = check_element_visibility(locator);
		System.out.println("current status is classis mode: " +status);
		if(status==true)
		{
